...
const findGame = (array, id) =>
  array.filter((game) => game.find((player) => player.id === id))[0];
const findGameIndex = (array, id) =>
  array.findIndex((game) => game.find((player) => player.id === id));
const findPlayerIndex = (array, id) =>
  array.findIndex((player) => player.id === id);
const findPlayerAltIndex = (array, id) =>
  array.findIndex((player) => player.id !== id);
const findOpponentScore = (array, gameIndex, playerAltIndex) =>
  array[gameIndex][playerAltIndex].score;
const hasOpponentPlayed = (array, gameIndex, playerAltIndex) =>
  array[gameIndex][playerAltIndex].played;
...
const validateScore = (newScore, altScore, winningScore, played) => {
  ...
  let check4 =
    newScore > altScore && altScore >= winningScore
      ? !(Math.abs(newScore - altScore) > 2)
      : true;
  let check5 =
    newScore > altScore && altScore < winningScore
      ? !(newScore > winningScore && Math.abs(newScore - altScore) > 2)
      : true;
  let check6 =
    altScore > winningScore ? Math.abs(newScore - altScore) === 2 : true;
