...
const score = (state, { newScore, id }) => {
  ...
  let completeCheck = updatedGames
    .flatMap((game) => game.map((player) => player.played))
    .some((el) => !el);
...
const history = (state) => {
  return {
    ...state,
    history: [...state.history, state.games],
  };
};
...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case "START":
      return newTournament(state, action);
    case "SETTINGS":
      return viewSettings(state, action);
    case "TOURNAMENT":
      return viewGames(state, action);
    case "RESULTS":
      return viewResults(state, action);
    case "SCORE":
    ...
    case "END_TOURNAMENT":
      return endTournament(state, action);
    default:
      return state;
  }
};
